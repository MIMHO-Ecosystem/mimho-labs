...
contract MIMHOVesting is Ownable2StepLite, ReentrancyGuardLite {
    ...
    modifier onlyDAOorOwner() {
        if (daoActivated) {
            require(msg.sender == dao, "ONLY_DAO_OR_OWNER");
        } else {
            require(msg.sender == owner, "ONLY_DAO_OR_OWNER");
        }
        _;
    }
    ...
    function finalize() external onlyDAOorOwner onlyConfigMode {
        ...
        _emitHubEvent(A_FINALIZED, msg.sender, 0, abi.encode(ecosystemReceiver));
    }
    ...
    function contractStatus() external view returns (
        ...
        return (version, paused, finalized, address(MIMHO), address(registry), dao, daoActivated, ecosystemReceiver);
    }
